...
sudo su {user_name} -c "mkdir -p /home/{user_name}/{project_name}/{exp_name}/weights"
...
sudo su ubuntu -c 'aws s3 sync s3://{exp_name}/weights /home/{user_name}/{project_name}/{exp_name}/weights'
...
cp runs/run_{exp_name}.py .
THEANO_FLAGS='floatX=float32,mode=FAST_RUN,allow_gc=False,device=gpu,lib.cnmem=0.9' python run_{exp_name}.py 2>&1 | multilog t s100000 '!tai64nlocal|gzip' ./logs &

watch -n 60 "sudo su {user_name} -c 'aws s3 sync /home/{user_name}/{project_name}/{exp_name}/weights s3://{exp_name}/weights' && sudo su {user_name} -c 'aws s3 sync /home/{user_name}/{project_name}/logs s3://{exp_name}/logs' && echo \`date\` >> /home/{user_name}/last_sync" &
        """.format(**kargs)
